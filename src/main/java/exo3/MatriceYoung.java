...
public class MatriceYoung  {
    ...
    private int n;
    private int m;
    private List<List<Integer>> tableauYoung;
    public MatriceYoung(int n , int m ) {
        this.n=n;
        this.m = m;
        tableauYoung=new ArrayList<>(n);
        for (int i = 0; i < n; i++){
            tableauYoung.add(new ArrayList<Integer>(m));
            for (int j = 0; j < m; j++){
    ...
    private int getElement(int i,int j){
        return  tableauYoung.get(i).get(j);
    }
    private void setElement(int i, int j , int valeur){
         tableauYoung.get(i).set(j,valeur);
    }
}